			...
			<DataCard {...cardProps} metricValue={undefined}>
				<div style={{ alignItems: "center", display: "flex", gap: 32 }}>
					<svg height={size} style={{ overflow: "visible" }} viewBox={`0 0 ${size} ${size}`} width={size}>
						{segments.map((seg, i) => {
							const prevPercentages = segments.slice(0, i).reduce((a, s) => a + s.percentage, 0);
							const segStart = startAngle + (prevPercentages / 100) * totalAngle;
							const segEnd = startAngle + ((prevPercentages + seg.percentage) / 100) * totalAngle;

							const segDelay = sweepStart + i * 5;
							const segProgress = interpolate(frame, [segDelay, sweepEnd], [segStart, segEnd], {
						...
						<text
							...
							y={centerY - 5}
						>
							{metricValue !== undefined ? Math.round(centerCount).toLocaleString() : ""}
						</text>
