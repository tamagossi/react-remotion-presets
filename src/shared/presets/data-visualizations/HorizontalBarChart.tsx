...
export const HorizontalBarChart: React.FC<HorizontalBarChartProps> = ({
  ...
  const maxValue = useMemo(() => Math.max(...allValues) * 1.1, [allValues]);
    ...
    >
      ...
      >
        ...
        {labels.map((label, labelIndex) => {
          ...
          const groupHeight = (drawHeight / labels.length) * 0.7;
          ...
          return (
            <g key={`group-${labelIndex}`}>
              ...
              {data.map((series, seriesIndex) => {
                ...
                const progressIndex = labelIndex * data.length + seriesIndex;
                const progress = barProgress[progressIndex];
                const barW =
                  (dataPoint.value / maxValue) * drawWidth * progress;
