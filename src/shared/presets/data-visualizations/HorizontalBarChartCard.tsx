...
export const HorizontalBarChartCard: React.FC<HorizontalBarChartCardProps> = ({
	...
	const trackBg = trackColor ?? (cardProps.theme === "light" ? "#e5e5e5" : "#333333");
			...
			<DataCard {...cardProps} metricValue={metricValue}>
				<div style={{ width: chartWidth }}>
					{data.map((item, i) => {
						...
						const barProgress = interpolate(frame, [itemDelay, barEnd], [0, 1], {
							easing: Easing.out(Easing.quad),
							extrapolateLeft: "clamp",
							extrapolateRight: "clamp",
						});

						const labelOpacity = interpolate(frame, [itemDelay, itemDelay + 8], [0, 1], {
							extrapolateLeft: "clamp",
							extrapolateRight: "clamp",
						});

						const percentage = Math.round((item.value / (item.maxValue ?? maxValue)) * 100);
