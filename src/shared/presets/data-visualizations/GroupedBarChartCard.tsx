...
export const GroupedBarChartCard: React.FC<GroupedBarChartCardProps> = ({
	...
	const groupWidth = data[0]?.values.length * barWidth + (data[0]?.values.length - 1) * barGap;
				...
				>
					...
					{data.map((item, groupIndex) => {
						...
						return item.values.map((value, barIndex) => {
							...
							const barH = (value / maxValue) * (chartHeight - padding.top - padding.bottom);
							const barDelay = groupDelay + barIndex * 1.5;

							const barProgress = interpolate(frame, [barDelay, barEnd], [0, 1], {
